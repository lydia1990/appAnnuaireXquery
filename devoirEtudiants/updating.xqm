...
declare
 ...
 %rest:query-param("adresse","{$adresse}","")
  %rest:query-param("birthday","{$birthday}","")
   %rest:query-param("food","{$food}","")
 
 %output:method("xhtml")
 function page:update($nom, $prenom, $adresse,$birthday,$food)
 {
   let $entry := 
    ...
    let $validate := validate:xsd-info($entry,'/basex/webapp/fichClass.xsd')
...
declare
 ...
 %output:method("xhtml")
 function page:updateEntry($id,$nom, $prenom, $adresse,$birthday,$food)
 {
   let $entry := 
