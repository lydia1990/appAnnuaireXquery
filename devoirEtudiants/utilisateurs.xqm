...
declare
 ...
 function page:login()
 {
   xls:templateLogin(
     ...
     <h1>Login<img width="100" src="https://www.champion-direct.com/206614-tm_large_default/cadenas-de-consignation-serie-74.jpg"/></h1>
      <p><label>Login</label><input type="text" name="login"/></p>
      <p><label>Mot de passe</label><input type="password" name="password"/></p>
 ...
 function page:admin()
 {
  xls:template(
   ...
   for $x in db:open('Tilde_utilisateurs')/bdd/entry
    return 
     <tr><td>{$x/login}</td><td>{$x/level}</td><td><a href="/delusers/{data($x/@id)}">Supprimer <img width="100" src="https://www.informatique.us/photo/corbeille.jpg"/></a></td></tr> 
    else
